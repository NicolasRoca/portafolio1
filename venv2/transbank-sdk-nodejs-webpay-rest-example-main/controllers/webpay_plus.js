...
/* exports.getToken = asyncHandler(async function (request, response, next) {
  let buyOrder = "O-" + Math.floor(Math.random() * 10000) + 1;
  let sessionId = "S-" + Math.floor(Math.random() * 10000) + 1;
  let amount = 1000; // Aquí puedes definir un valor fijo o recibirlo desde el request.body
  ...
  try {
    ...
    response.json({ token, url }); // Enviar token y url al frontend
  } catch (error) {
    console.error("Error creating transaction:", error);
    if (!response.headersSent) {
      return next(error); // Manejar el error adecuadamente
    }
  }
});
 */
